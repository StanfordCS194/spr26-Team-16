...
class Settings(BaseSettings):
    ...
    # Retention policy knobs (ARCHITECTURE.md §13). All windows in days.
    retention_soft_delete_days: int = 30
    retention_failed_push_days: int = 7
    retention_audit_log_days: int = 90
    retention_revoked_token_days: int = 365
    # Stuck-push detection: pushes in pending/processing older than this are flagged.
    stuck_push_minutes: int = 5
    # User-deletion cascade: hard cutoff to complete the purge after the request.
    user_deletion_grace_days: int = 30
    # Admin scope name on JWT/api_token for retention endpoints.

...
class TranscriptStorageService:
    ...
    async def delete_transcript(self, storage_path: str) -> None:
        """Best-effort delete. Missing-object is not an error — the retention
        job re-runs daily and partial state is fine."""
        if self._supabase_client:
            self._supabase_client.storage.from_(self._bucket).remove([storage_path])
            return
        target = self._local_root / storage_path
        if target.exists():
            target.unlink()

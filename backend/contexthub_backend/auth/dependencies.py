...
def require_admin_scope(
    ...
    """Guard that gates service-level admin endpoints behind the admin scope.

    JWT callers receive ALL_SCOPES at sign-in; api-token callers must hold
    settings.admin_scope explicitly. The scope name is configurable so
    operators can rename it without code changes.
    """
    if not user.has_scope(settings.admin_scope):
        raise ForbiddenError(
            f"token missing required scope: {settings.admin_scope}"
        )
    return user

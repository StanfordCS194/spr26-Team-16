...
Version control for LLM-assisted knowledge work. Push completed conversations to
a repository, search them semantically, and pull context back into new ones.

ContextHub is a monorepo: a FastAPI backend with a background worker, a Next.js
dashboard, and a Manifest V3 Chrome extension that adds a sidebar to `claude.ai`.
Auth is Supabase Google OAuth; conversations are summarized and embedded for
semantic search.
